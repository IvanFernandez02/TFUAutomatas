...
/**
 * Implementacion concreta: Parser recursivo descendente.
 * Patron Bridge: implementacion formal del analisis sintactico.
 */
@Component
public class FormalSyntacticParser {
    ...
    public AstNode parse(List<Token> tokens) {
        ...
        if (tokens.isEmpty()) {
            throw new SyntaxException("Error sintactico: consulta vacia");
        }
        ...
        return switch (peek().type()) {
            ...
            default -> throw new SyntaxException(
                    "Error sintactico: sentencia no reconocida near '" + peek().value() + "'");
        };
    }
    ...
    private String parseOperator() {
        ...
        return switch (token.type()) {
            ...
            default -> throw new SyntaxException(
                    "Error sintactico: operador invalido near '" + token.value() + "'");
        };
    }
    ...
    private Value parseValue() {
        ...
        return switch (token.type()) {
            ...
            default -> throw new SyntaxException(
                    "Error sintactico: valor invalido near '" + token.value() + "'");
        };
    }
    ...
    private String expectIdentifier() {
        ...
        if (!"IDENTIFICADOR".equals(token.type())) {
            throw new SyntaxException(
                    "Error sintactico: se esperaba identificador near '" + token.value() + "'");
        }
        advance();
        return token.value();
    }
    ...
    private void expect(String type) {
        ...
        if (!type.equals(token.type())) {
            throw new SyntaxException(
                    "Error sintactico: se esperaba " + type + " near '" + token.value() + "'");
        }
        advance();
    }
    ...
    private Token peek() {
        if (index >= tokens.size()) {
            throw new SyntaxException("Error sintactico: entrada incompleta o invalida");
        }
